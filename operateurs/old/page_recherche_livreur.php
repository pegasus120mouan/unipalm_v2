...
$stmt = $conn->prepare(
    ...
    WHERE commandes.livreur_id LIKE :livreur_id
    ORDER BY commandes.date_commande DESC"
);

$rechercheAvecPourcentage = '%' . $recherche . '%';
$stmt->bindParam(':livreur_id', $rechercheAvecPourcentage, PDO::PARAM_STR);
$stmt->execute();
...
<div class="row">
  ...
  <div class="pagination-container bg-secondary d-flex justify-content-center w-100 text-white p-3">
    <?php if($page > 1): ?>
      <a href="?livreur_id=<?= urlencode($recherche) ?>&page=<?= $page - 1 ?>&limit=<?= $limit ?>" class="btn btn-primary">&lt;</a>
    <?php endif; ?>
