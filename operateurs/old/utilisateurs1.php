...
?>
        ...
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add-client">
          ...
          <table id="example1" class="table table-bordered table-striped">
                  ...
                  <td><?=$user['id']?></td>
                <td>
                <a href="client_profile.php?id=<?=$user['id']?>" class="edit"><img src="../pages/dossier_avatars/<?php echo $user['avatar']; ?>" alt="Logo" width="50" height="50"> </a>
                 </td>
                <td><?=$user['nom']?></td>
                <td><?=$user['prenoms']?></td>
                <td><?=$user['contact']?></td>
                ...
                <td><?=$user['role']?></td>
                    ...
                    <td class="actions">
                        <a href="utilisateurs_delete.php?id=<?=$user['id']?>" class="edit"><i class="fas fa-pen fa-xs" style="font-size:24px;color:blue"></i></a>
                        <a href="utilisateurs_delete.php?id=<?=$user['id']?>" class="trash"><i class="fas fa-trash fa-xs" style="font-size:24px;color:red"></i></a>
                    </td>
                  </tr>
                  <?php endforeach; ?>
                  </tbody>
            ...
            <form class="forms-sample" method="post" action="save_utilisateurs.php">
                <div class="card-body">
                 ...
                 <div class="form-group">
                        <label>Rôle</label>
                        <?php
                         echo  '<select id="select" name="role" class="form-control">';
                            while ($role_select = $role_selection->fetch(PDO::FETCH_ASSOC)) 
                          {
                          echo '<option value="' . $role_select['nom_role'] . '">' . $role_select['nom_role'] . '</option>';
                           }
                           echo '</select>'
                        ?>
                    </div>
