    ...
    <nav class="main-header navbar navbar-expand navbar-pro">
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item d-none d-lg-inline-block">
          <a href="approvisionnement.php" class="nav-link <?= basename($_SERVER['PHP_SELF']) == 'approvisionnement.php' ? 'active' : '' ?>">
...
<!-- Script d'initialisation des menus déroulants -->
<script>
$(document).ready(function() {
    console.log('🚀 Initialisation des menus déroulants');
    console.log('jQuery disponible:', typeof $ !== 'undefined');
    console.log('Nombre de menus trouvés:', $('.nav-sidebar .nav-item').length);
    console.log('Nombre de sous-menus trouvés:', $('.nav-treeview').length);
    
    // Réinitialiser les gestionnaires d'événements
    $('.nav-sidebar .nav-item > a').off('click.menu');
    
    // Gestion des menus déroulants de la sidebar
    $('.nav-sidebar .nav-item > a').on('click.menu', function(e) {
        var $this = $(this);
        var $parent = $this.parent('.nav-item');
        var $submenu = $this.next('.nav-treeview');
        var menuText = $this.find('p').first().text().trim();
        
        console.log('🖱️ Clic sur menu:', menuText);
        console.log('📁 Sous-menu trouvé:', $submenu.length > 0);
        console.log('🔗 Href:', $this.attr('href'));
        
        // Si c'est un lien avec sous-menu
        if ($submenu.length > 0 && $this.attr('href') === 'javascript:void(0)') {
            e.preventDefault();
            console.log('✅ Conditions remplies, traitement du menu');
            
            // Fermer les autres menus ouverts
            $('.nav-sidebar .nav-item.menu-open').not($parent).each(function() {
                console.log('🔒 Fermeture d\'un autre menu');
                $(this).removeClass('menu-open');
                $(this).find('.nav-treeview').slideUp(300);
            });
            ...
            // Toggle du menu actuel
            if ($parent.hasClass('menu-open')) {
                console.log('📤 Fermeture du menu:', menuText);
                $parent.removeClass('menu-open');
                $submenu.slideUp(300);
            } else {
                console.log('📥 Ouverture du menu:', menuText);
                $parent.addClass('menu-open');
                $submenu.slideDown(300);
            }
        } else {
            console.log('❌ Conditions non remplies');
            if ($submenu.length === 0) console.log('   - Pas de sous-menu trouvé');
            if ($this.attr('href') !== 'javascript:void(0)') console.log('   - Href incorrect:', $this.attr('href'));
        }
    });
